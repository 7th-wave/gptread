...
function filterLayersThatStartWithPrefix(){
    ...
    for (var i = 0; i < allLayersList.length; i++)
    {
        ...
        if (layerName.length > 0)
        {
            ...
            {
                if (app.activeDocument.layers[i].symbolItems.length >0)
                {
...
function generateCollection(docName) {
...
function showTrait(curLayer, layerNum) {
    ...
    curLayer.symbolItems[randomTrait].hidden = false;
...
for (var i=0; i < allChangingLayersList.length;i++)
{
    ...
    var atr = {
        ...
        traitName:allChangingLayersList[i].symbolItems[lastCreatedObj[i]].symbol.name.split(raritySuffix)[0],
...
function saveImage(docName) {
    ...
    var aiFile = new File(pathToSaveCollection + "/" + docName + '-' + currentItemGenerated + "_" + '.ai');
    ...
    for (var i = 0; i < num_artboards; i++ ) {
        ...
        var imgFile = new File(pathToSaveCollection + "/" + docName + '-' + currentItemGenerated + "_" + artboardName + '.png');
...
function checkMaxCollectionSize()
{
    for (var i = 0; i < allChangingLayersList.length; i++) {
        ...
        MAX_POSSIBLE_COLLECTION_SIZE *= allChangingLayersList[i].symbolItems.length;
    }
...
function calculateInitialRaritiesBasedOnCollectionSize(){
    ...
    for (var i = 0; i < allChangingLayersList.length; i++) {
        ...
        for (var j = 0; j < allChangingLayersList[i].symbolItems.length; j++) {
           
            //Initialize Created Nft Array
            createdTraitsArray.push(0);

            var layerName = allChangingLayersList[i].symbolItems[j].symbol.name;
            var layerRarity =extractRarityValue(layerName,allChangingLayersList[i].symbolItems.length);
...
{
   var MaxValidTrait = allChangingLayersList[layerNum].symbolItems.length-1;
...
function hideAllSubLayers(curLayer) {
    for (var i = 0; i < curLayer.symbolItems.length; i++) {
...
function maxCollectionSizeBasedOnRarities ()
{
    ...
    for (var i = 0; i < allChangingLayersList.length; i++)
    {
        ...
        for (var j = 0; j < raritiesNumArray[i].length; j++)
        {
         ...
         var avgR=100/allChangingLayersList[i].symbolItems.length;
